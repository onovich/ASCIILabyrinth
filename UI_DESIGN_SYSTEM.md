...
- Tool pages should call shared schema helpers directly through `SHARED_DATA` for `escapeHtml`, `escapeAttr`, `optionHtml`, `optionsHtml`, `optionsWithEmptyHtml`, `setValueByPath`, `getValueByPath`, `parseJson`, `stringifyJson`, `toIsoTimestamp`, `touchProject`, `countBy`, `clamp`, `createId`, and `degreesToRadians`.
